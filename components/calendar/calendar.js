...
Component({
	...
	pageLifetimes: {
		show() {
			this._openEvent(this.data.startDate, this.properties.date);
		}
	},
	/**
	 * 组件的方法列表
	 */
	methods: {
		_openEvent: function (startDateString, selectedDateString) {
			...
			if (selectedDateString && selectedDateString.length > 0) {
				selectedDate = new Date(selectedDateString.replace(/-/g, '/'))
			} else {
				selectedDate = startDate ? startDate : new Date()
			}
